...
export default function Auth() {
  ...
  useEffect(() => {
    ...
    if (firebaseConfigurationError && import.meta.env.DEV && !loading) {
      navigate('/monitor', { replace: true });
    }
  ...
  return (
    <main className="auth-entry-page relative h-screen w-full overflow-hidden bg-black">
      <MeshGradient className="absolute inset-0 h-full w-full" colors={['#000000', '#1a1a1a', '#2a2a2a', '#ffffff']} speed={0.8} backgroundColor="#000000" />

      <div className="pointer-events-none absolute inset-0">
        <div className="absolute left-1/3 top-1/4 h-32 w-32 animate-pulse rounded-full bg-gray-800/5 blur-3xl" />
        <div className="absolute bottom-1/3 right-1/4 h-24 w-24 animate-pulse rounded-full bg-white/5 blur-2xl" style={{ animationDelay: '1s' }} />
        <div className="absolute right-1/3 top-1/2 h-20 w-20 animate-pulse rounded-full bg-gray-900/10 blur-xl" style={{ animationDelay: '0.5s' }} />
      </div>

      <div className="absolute inset-0 flex items-center justify-center px-8">
        <div className="w-full text-center">
          <h1 className="title-text mb-3">TokenGuard</h1>
          <p className="subtitle-text mb-3">{isRegistering ? 'Create your TokenGuard account.' : 'A protected coding experience.'}</p>

          <div className="sign-in-form-container">
            <form className="sign-in-form" onSubmit={handleSubmit}>
              <div className="input-wrapper">
                <Mail className="input-icon" size={18} />
                <input type="email" placeholder="Email address" className="form-input" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" required />
              </div>
              ...
              <button type="button" className="google-button" onClick={() => void handleGoogleSignIn()} disabled={isWorking}>
              ...
              <p className="register-text">
                {isRegistering ? 'Already have an account? ' : 'New to TokenGuard? '}
                <button type="button" className="register-link" onClick={() => { setIsRegistering((current) => !current); setErrorMessage(null); }} style={{ background: 'transparent', border: 0, cursor: 'pointer', padding: 0 }}>
